...
class RageQuit
{
    const string pattern = @"((?<str>[^0-9]+)(?<num>\d+))";

    static void Main(string[] args)
    {
        StringBuilder result = new StringBuilder();

        foreach (Match match in Regex.Matches(Console.ReadLine().ToUpper(), pattern))
        {
            result.Append(string.Concat(Enumerable.Repeat(match.Groups["str"].Value, int.Parse(match.Groups["num"].Value))));
        }
